...
class CropPhenology:
    ...
    def extractSeasonDates(self, timeseries, args):
        ...
        if timeseries is None:
            ...
            mMax = self.getLocalMax(timeseries, pd.Timestamp(year,args.mStart.months,args.mStart.days), pd.Timestamp(year,args.mEnd.months,args.mEnd.days))
            dmMax = mMax['Times']
            ymMax = mMax['Greenness']

            # Get the start of season dates
            sos = self.getStartOfSeason(timeseries, pd.Timestamp(year,args.sStart.months,args.sStart.days), pd.Timestamp(year,args.sEnd.months,args.sEnd.days), float(args.tSos), float(ymMax))
            result['sos']['time'] = sos[3].strftime('%Y-%m-%d')
            result['sos']['greenness'] = sos[2]

            # Get the end of season dates
            eos = self.getEndOfSeason(timeseries, pd.Timestamp(year,args.eStart.months,args.eStart.days), pd.Timestamp(year,args.eEnd.months,args.eEnd.days), float(args.tEos), float(ymMax))
